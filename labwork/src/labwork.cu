...
int main(int argc, char **argv)
{
    ...
    case 6:
        if (argc < 5)
        {
            printf("Usage: labwork <lwNum> <inputImage> <method> <detailArg>\n");
            printf("  lwNum   labwork number\n");
            printf("  inputImage  the input file name, in JPEG format\n");
            printf("  method    1 : binari  2 : brightness  3 : blending\n");
            printf("  detailArg method=1 : threshold  method=2 : brightnessCoef method=3 : second JPEG image\n");
            return 0;
        }
        else
        {
            std::string inputFilename;
            inputFilename = std::string(argv[4]);
            labwork.loadInputImage2(inputFilename);
        }
        labwork.labwork6_GPU(atoi(argv[3]), atoi(argv[4]));
        printf("labwork %d ellapsed %.1fms\n", lwNum, timer.getElapsedTimeInMilliSec());
...
void Labwork::labwork6_GPU(int method, int detailArg)
{
    ...
    if (method == 1) // Launch Binari kernel
    {
        ...
        binari <<< dimGrid, dimBlock>>>(devGray, devBin, inputImage->width, inputImage->height, detailArg);
    }
    else if(method == 2) // Launch Brightness kernel
    {
        grayscale <<< dimGrid, dimBlock>>>(devInput, devGray, inputImage->width, inputImage->height);
        brightness <<< dimGrid, dimBlock>>>(devGray, devBright, inputImage->width, inputImage->height, detailArg);
    }
    else if (method == 3) // Launch Blending kernel
    {
        blending <<< dimGrid, dimBlock>>>(devInput, devInput2, devBlend, 0.50, inputImage->width, inputImage->height, inputImage2->width, inputImage2->height);
    }
...
void Labwork::labwork9_GPU()
{
    ...
    // Process data
    grayscale <<< nbBlock * 1024, dimBlock>>>(devInput, devGray, inputImage->width, inputImage->height);
